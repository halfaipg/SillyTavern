...
router.post('/text-workers', jsonParser, async (request, response) => {
    try {
        ...
        const fetchResult = await fetch('https://api.aipowergrid.io/api/v2/workers?type=text', {
            headers: {
                'Client-Agent': agent,
            },
        });
...
router.post('/text-models', jsonParser, async (request, response) => {
    try {
        ...
        const agent = await getClientAgent();
        const fetchResult = await fetch('https://api.aipowergrid.io/api/v2/status/models?type=text', {
            headers: {
                'Client-Agent': agent,
            },
        });
...
router.post('/status', jsonParser, async (_, response) => {
    try {
        const agent = await getClientAgent();
        const fetchResult = await fetch('https://api.aipowergrid.io/api/v2/status/heartbeat', {
            headers: {
                'Client-Agent': agent,
            },
        });
...
router.post('/cancel-task', jsonParser, async (request, response) => {
    try {
        ...
        const agent = await getClientAgent();
        const fetchResult = await fetch(`https://api.aipowergrid.io/api/v2/generate/text/status/${taskId}`, {
            method: 'DELETE',
            headers: {
                'Client-Agent': agent,
            },
...
router.post('/generate-text', jsonParser, async (request, response) => {
    ...
    const url = 'https://api.aipowergrid.io/api/v2/generate/text/async';
    const agent = await getClientAgent();
